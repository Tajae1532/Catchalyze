...
-- Modify existing type column to use enum (if needed)
DO $$
BEGIN
    -- First check if the column is already using the enum type
    IF EXISTS (
...
-- Modify existing severity column to use enum (if needed)
DO $$
BEGIN
    -- First check if the column is already using the enum type
    IF EXISTS (
